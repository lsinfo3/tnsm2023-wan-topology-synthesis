...
for name in names:
    ...
    for sample_weights, color in zip(sample_weights_list, colors):
        for k in range(100):
            for s in range(10):
                    with open(path +"/synth_sample_"+str(k)+"_"+str(s)+"_"+color+"_"+str(int(sample_weights))+"_.pkl","rb") as f:
                        full_graph = pickle.load(f)
                        G = graph2omnet(full_graph, "WANGAN_"+color+"_"+ name+ "_sample_"+str((k+1)+(s)*100))